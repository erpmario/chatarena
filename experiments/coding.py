...
class IterativeCoding(Environment):
	...
	def reset(self):
		...
		self._moderator_speak(
			...
			f"\n Write some testcases and then an actual function that implement the task. Everything should be in a single code block",
			visible_to = "coder",
		)
	...
	def process_broken(self):
		self._moderator_speak("The process is broken. Please restart the game.")
		self._terminal = True
		observation = self.get_observation(self.get_next_player())
		return TimeStep(
			observation = observation,
	...
	def step(self, player_name: str, action: str) -> TimeStep:
		...
		if self.phase == "verify":
			self._moderator_speak(
				f"Here's the outputs: {interpreter_output}. Is the code correct? Output with json format.",
				visible_to = "verifier",
			)
		elif self.phase == "iterate":
			self._moderator_speak(
				"Now iterate your code with feedbacks. First think about why and then write the new code.",
				visible_to = "coder",
			)
...
if __name__ == "__main__":
    ...
    You are a verifier. You are going to verify if the code is correct or not according to the interpreter outputs.
	...
	verifier = Player(
		"verifier",
		role_desc = verifier_role_description,
		backend = OpenAIChat(max_tokens = 1024, model = "gpt-4"),
	)
