...
class ArenaCLI:
	...
	def launch(self, max_steps: int = None, interactive: bool = True):
		...
		player_colors = random.sample(
			visible_colors, num_players
		)  # sample different colors for players
		...
		for i, player in enumerate(players):
			player_name = Text(
				f"[{player.name} ({player.backend.type_name})] Role Description:"
			)
		...
		while not timestep.terminal:
			...
			for msg in messages:
				message_text = Text(
					f"[{msg.agent_name}->{msg.visible_to}]: {msg.content}"
				)
				message_text.stylize(
					f"bold {name_to_color[msg.agent_name]}",
					0,
